...
fn main() {
    ...
    std::fs::write("output/heatmap_correlation.svg", fig1.to_svg()).expect("Failed to write correlation heatmap to file");
    println!("Correlation heatmap saved to heatmap_correlation.svg");
    
    // Example 2: Temperature data (simulated)
    let temp_data: Vec<Vec<f64>> = (0..8).map(|i| {
        (0..12).map(|j| {
    ...
    std::fs::write("output/heatmap_temperature.svg", fig2.to_svg()).expect("Failed to write temperature heatmap to file");
    ...
    std::fs::write("output/heatmap_intensity.svg", fig3.to_svg()).expect("Failed to write intensity heatmap to file");
