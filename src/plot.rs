...
impl Plot {
    ...
    pub fn histogram(data: &[f64], bins: usize) -> Self {
        ...
        let bin_centers: Vec<f64> = bin_edges.windows(2)
            .map(|w| (w[0] + w[1]) / 2.0)
            .collect();
    ...
    pub fn to_svg(&self, x_min: f64, x_max: f64, y_min: f64, y_max: f64, 
        ...
        if matches!(self.plot_type, PlotType::Pie | PlotType::BoxPlot | PlotType::Heatmap | PlotType::Violin | PlotType::Contour) {
        ...
        match self.plot_type {
            ...
            PlotType::Histogram => {
                svg.push_str(&self.generate_histogram_svg(x_min, x_max, y_min, y_max, 
                                                         plot_width, plot_height, &color_str));
            },
            PlotType::Pie => {
                svg.push_str(&self.generate_pie_svg(x_min, x_max, y_min, y_max, 
                                                   plot_width, plot_height, &color_str));
            },
            PlotType::BoxPlot => {
                svg.push_str(&self.generate_boxplot_svg(x_min, x_max, y_min, y_max, 
                                                        plot_width, plot_height, &color_str));
            },
            PlotType::Heatmap => {
                svg.push_str(&self.generate_heatmap_svg(x_min, x_max, y_min, y_max, 
                                                        plot_width, plot_height, &color_str));
            },
            PlotType::Violin => {
                svg.push_str(&self.generate_violin_svg(x_min, x_max, y_min, y_max, 
                                                       plot_width, plot_height, &color_str));
            },
            PlotType::Contour => {
                svg.push_str(&self.generate_contour_svg(x_min, x_max, y_min, y_max, 
                                                        plot_width, plot_height, &color_str));
            },
        }
    ...
    fn generate_histogram_svg(&self, x_min: f64, x_max: f64, y_min: f64, y_max: f64, 
                             plot_width: f64, plot_height: f64, color: &str) -> String {
        let mut svg = String::new();
        ...
        let data_range = if self.x_data.len() > 1 {
            ...
            let min_diff = sorted_x.windows(2)
                .map(|w| w[1] - w[0])
    ...
    fn generate_pie_svg(&self, _x_min: f64, _x_max: f64, _y_min: f64, _y_max: f64, 
                       plot_width: f64, plot_height: f64, color: &str) -> String {
        let mut svg = String::new();
        ...
        for (i, &value) in self.x_data.iter().enumerate() {
            ...
            if percentage >= 5.0 { // Only show labels for slices >= 5%
                svg.push_str(&format!(
    ...
    fn generate_boxplot_svg(&self, _x_min: f64, _x_max: f64, y_min: f64, y_max: f64, 
                           plot_width: f64, plot_height: f64, color: &str) -> String {
        let mut svg = String::new();
        ...
        let lower_whisker = sorted_data.iter().find(|&&x| x >= lower_fence).copied().unwrap_or(sorted_data[0]);
        let upper_whisker = sorted_data.iter().rev().find(|&&x| x <= upper_fence).copied().unwrap_or(sorted_data[n-1]);
        ...
        let y_to_svg = |y: f64| -> f64 {
            plot_height - map_range(y, y_min, y_max, 0.0, plot_height)
        };
    ...
    fn generate_heatmap_svg(&self, _x_min: f64, _x_max: f64, _y_min: f64, _y_max: f64, 
                            plot_width: f64, plot_height: f64, _color: &str) -> String {
        let mut svg = String::new();
    ...
    fn generate_violin_svg(&self, _x_min: f64, _x_max: f64, y_min: f64, y_max: f64, 
                           plot_width: f64, plot_height: f64, color: &str) -> String {
        let mut svg = String::new();
    ...
    fn generate_contour_svg(&self, x_min: f64, x_max: f64, y_min: f64, y_max: f64, 
                            plot_width: f64, plot_height: f64, _color: &str) -> String {
        let mut svg = String::new();
